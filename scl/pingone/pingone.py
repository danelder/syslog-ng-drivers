...
class PingAdmin(syslogng.LogFetcher):
    ...
    def init(self, options):
        ...
        if "timeout" in options:
            try:
                ...
                self.logger.debug("Setting timeout to %i", self.timeout)
            except Exception as ex:
                self.logger.error("Invalid timeout specified (%s) : %s", options["timeout"], ex)
                self.timeout = 120

        # Ensure stats output file is writable if configured
        self.write_stats = False
        if "stats_file" in options:
            try:
                fh = open(options["stats_file"], "a")
                fh.close()
                self.write_stats = options["stats_file"]
                self.logger.debug("Using %s for statistics output", self.write_stats)
            except Exception as ex:
                self.logger.error("Error appending to stats file (%s) : %s", options["stats_file"], ex)
        ...
        if "disk_buffer" in options:
            ...
            try:
                ...
                # Write stats output if configured
                if self.write_stats:
                    try:
                        self.stats_file.write("Loaded %i events from disk buffer %s\n" % (len(self.logs), self.buffer))
    ...
    def process_message(self, log):
        """
        Parse a single log message and extract timestamp and program field
        """
        ...
        if "recorded" in log:
            try:
                # Timestamp formats aren't consistent
                timestamp = datetime.strptime(log['recorded'], "%Y-%d-%mT%H:%M:%S.%f%z")
                ...
                self.logger.error("Unknown error extracting timestamp from %s : %s", log['recorded'], ex)
    ...
    def fetch(self):
        ...
        try:
            response = requests.get(subscription_url, headers=headers, timeout=self.timeout)
            ...
            # If this isn't a 504 error
                ...
                if response.status_code == 200:
                    ...
                    try:
                        ...
                        self.logger.error("Server response wasn't valid json : %s", response.text)
                        return syslogng.LogFetcher.FETCH_ERROR, "Invalid json response - %s", response.text
                    ...
                    if self.write_stats:
                        try:
                            self.stats_file.write("Retrieved %i messages from Ping API fetch\n" % len(self.logs))
                            self.stats_file.flush()
                        except Exception as ex:
                            self.logger.error(ex)
                    ...
                    if self.logs:
                        ...
                        # Process message from retrieved events
                        log = self.logs.pop()
                        msg = self.process_message(log)
                        return syslogng.LogFetcher.FETCH_SUCCESS, msg
    ...
    def open(self):
        ...
        # Open stats file if configured for appending
        if self.write_stats:
            try:
                self.stats_file = open(self.write_stats, "a")
            except Exception as ex:
                self.logger.error("Error appending to stats file (%s) : %s", self.write_stats, ex)
    ...
    def close(self):
        ...
        # Flush stats to disk if configured
        if self.write_stats:
            try:
                self.stats_file.close()
            except Exception as ex:
                self.logger.error("Error closing stats file (%s) : %s", self.write_stats, ex)
    ...
    def deinit(self):
        ...
        if self.write_stats:
            try:
                self.stats_file.write("Deinitializing with %i events in memory buffer\n" % len(self.logs))
                self.stats_file.flush()
            except Exception as ex:
                self.logger.error(ex)
            ...
            try:
                ...
                try:
                    with open(self.buffer, 'a') as filebuffer:
                        ...
                        while self.logs:
                            message = json.dumps(self.logs.pop())
