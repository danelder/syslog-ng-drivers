...
class ProofpointOnDemand(syslogng.LogSource):
    ...
    - The access token is being used by another session""")
        ...
        # Catch all for unknown error type
            ...
            if self.proxy_host is not False:
                self.logger.info("This may be the result of invalid proxy settings (%s://%s:%i)", \
    ...
    def init(self, options): # optional
        ...
        if "proxy_host" in options:
            ...
            self.logger.info("Setting proxy_host to %s", self.proxy_host)

            # Set proxy_port setting
            if "proxy_port" in options:
                try:
                    self.proxy_port = int(options["proxy_port"])
                except Exception:
                    self.logger.error("proxy_port must be an integer value : %s", options["proxy_port"])
                    self.logger.info("Defaulting to port 3128 for proxy")
                    self.proxy_port = 3128
            else:
                self.logger.info("No proxy_port specified in driver configuration, using 3128")
                self.proxy_port = 3128

            # Set proxy_type
            if "proxy_type" in options:
                self.proxy_type = options["proxy_type"].lower()
                if self.proxy_type == "http" or "socks4" in self.proxy_type or "socks5" in self.proxy_type:
                    self.logger.info("Setting %s as proxy_type", self.proxy_type)
                else:
                    self.logger.warning("Invalid proxy type specified (%s), defaulting to http", self.proxy_type)
                    self.proxy_type = "http"
            else:
                self.logger.info("No proxy type specified, defaulting to http")
                self.proxy_type = "http"
        ...
        if "backfill_hours" in options:
            try:
                ...
                if "backfill_start" not in options:
                    # Round down to hour boundary and get datetime for backfill_start hours ago
                    now = datetime.utcnow().replace(microsecond=0, second=0, minute=0)
                    delta = timedelta(hours = self.backfill_hours)
                    start_time = now - delta

                    # Set last_run to start_time if older
                    if start_time < self.last_run:
                        self.last_run = start_time
                    
                    self.backfill_start = start_time.strftime("%Y-%m-%dT%H:%M:%S") + "-0000"
                    self.logger.info("Setting fetch window back %i hours to %s", self.backfill_hours, self.backfill_start)
            except Exception:
        ...
        if "backfill_start" in options:
            try:
                # Make sure this is a valid datetime format and round down the hour
                backfill_start = datetime.strptime(options["backfill_start"], "%Y-%m-%dT%H:%M:%S-0000").replace(microsecond=0, second=0, minute=0)

                # Convert back to string for query
                self.backfill_start = backfill_start.strftime("%Y-%m-%dT%H:%M:%S") + "-0000"
    ...
    def run(self): # mandatory
        ...
        while not self.exit:
            ...
            if self.hourly_fetch:
                ...
                if self.exit:
                    self.logger.debug("De-scheduling log fetch for %s", self.last_run)
                    break
                ...
                self.persist["last_run"] = self.backfill_start
            ...
            # Create a sinceTime if backfill_hours is set
                ...
                # Get current time - backfill_hours hours rounded down to hour boundary
                now = datetime.utcnow().replace(microsecond=0, second=0, minute=0)
                delta = timedelta(hours = self.backfill_hours)
                start_time = now - delta
            ...
            # No proxy is needed
            if self.proxy_host is False:
                # Don't verify certs or use proxy
                if self.ssl_verify is False:
                    self.wsapp.run_forever(sslopt={"cert_reqs": ssl.CERT_NONE}, ping_interval=10, skip_utf8_validation=True)
                # Verify certs and use proxy
                else:
                    self.wsapp.run_forever(sslopt={"ca_certs": certifi.where()}, ping_interval=10, skip_utf8_validation=True)
            else:
                 # Don't verify certs but do use proxy
                if self.ssl_verify is False:
            ...
            if self.backfill_start and self.end_timestamp and self.exit is False:
                ...
                # Update next_run for next loop of hourly_fetch
                    ...
                    if self.error:
                        self.logger.warning("Will re-run last fetch due to errors, event duplication possible")
                    else:
                        self.last_run = self.next_run
                        self.next_run = self.last_run + timedelta(hours=1)
                        self.logger.debug("Setting persistence for last_run to %s", self.end_timestamp)
                        self.persist["last_run"] = self.end_timestamp

            # If this was a backfill_hours search, make sure we start new searches at current time
            elif self.backfill_hours > 0 and self.exit is False and self.error is False:
            ...
            if not self.exit and not self.backfill_start:
                self.logger.warning("Websocket connection lost, event duplication is likely")

            if self.exit and self.hourly_fetch:
                self.logger.debug("Interrupted wss pull from %s to %s, duplicate events likely", self.backfill_start, self.end_timestamp)
